...
AppDelegate::AppDelegate() {
    ...
    if (hModule != NULL)
    {
        ...
        if (pos != path.npos)
        {
            ...
            FileUtils::getInstance()->addSearchPath(StringUtils::format("%s%s", path.c_str(), "\\res"));
        }
    }
#else
...
bool AppDelegate::applicationDidFinishLaunching() {
    ...
    if(!glview) {
        glview = GLViewImpl::createWithRect("Load3D", Rect(0, 0, 960, 640));
    ...
    auto scene = GameScene3D::getInstance();
